...
impl From<::ate::error::ChainCreationError> for CoinError {
...
impl From<::ate::error::CommitError> for CoinError {
...
impl From<CoinCollectFailed> for CoinError {
    ...
    fn from(err: CoinRotateFailed) -> CoinError {
        match err {
            ...
            CoinRotateFailed::OperatorNotFound => {
                CoinErrorKind::CoreError(CoreErrorKind::OperatorNotFound).into()
            }
            ...
            CoinRotateFailed::AccountSuspended => {
                CoinErrorKind::CoreError(CoreErrorKind::AccountSuspended).into()
            }
            CoinRotateFailed::InternalError(code) => {
                CoinErrorKind::CoreError(CoreErrorKind::InternalError(code)).into()
            }
        }
    }
