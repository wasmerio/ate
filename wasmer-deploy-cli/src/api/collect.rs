...
impl DeployApi {
    pub(crate) async fn __coin_collect_internal(
        &mut self,
        ...
        for confirm in query.confirmations.iter() {
            if let Err(err) = self
                .record_activity(HistoricActivity::DepositCompleted(
                    activities::DepositCompleted {
    ...
    pub(super) async fn __collect_coins(&mut self) -> Result<(), WalletError> {
        ...
        for currency in currencies {
            ...
            while ancestors.len() > 0 {
                ...
                query.pending_deposits.iter().for_each(|o| {
                    pending_deposits.insert(o.owner.clone());
                });
