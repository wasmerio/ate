...
pub enum SshWizardStep
{
    ...
    Sudo,
    /// If the user is not known to Tokera then they enter a
...
impl SshWizardState
{
    ...
    {
        msg
            .replace("{email}", self.email.clone().unwrap_or("none".to_string()).as_str())
            .replace("{terms}", self.needed_terms.clone().unwrap_or("none".to_string()).as_str())
            .replace("{qr_code}", self.qr_code.clone().unwrap_or("none".to_string()).as_str())
    }
}
        ...
        match self.step.clone() {
            SshWizardStep::Init => {
                if self.state.session.is_some() {
                    ...
                    if regex.is_match(username.as_str()) == false {
                        warn!("user check failed: username was invalid (user={})", username);
                        self.state.goodbye_message = Some(CConst::SSH_INVALID_USERNAME.to_string());
                        self.step = SshWizardStep::Terminate;
                        return;
                    }
                    ...
                    self.step = match user_exists(&self.registry, &self.auth, &mut self.state).await {
                        ...
                        UserExistsResult::NotFound => {
                            info!("user does not exist (user={})", self.state.email.clone().unwrap());
            ...
            SshWizardStep::Login => {
                ...
                self.step = match login(&self.registry, &self.auth, &mut self.state).await {
                    LoginResult::LoginWithUser | LoginResult::LoginWithPublicKey => {
                        info!("login successful (user={})", self.state.email.clone().unwrap());
                        SshWizardStep::Sudo
                    },
                    LoginResult::Unverified => {
                        info!("login successful - must verify (user={})", self.state.email.clone().unwrap());
                        SshWizardStep::Verify
                    },
                    LoginResult::Unregistered => {
                        info!("login failed - must sign-up (user={})", self.state.email.clone().unwrap());
                        SshWizardStep::SignUp
                    },
            ...
            SshWizardStep::Sudo => {
                ...
                self.step = match sudo(&self.registry, &self.auth, &mut self.state).await {
                    SudoResult::Success => {
                        info!("login sudo successful (user={})", self.state.email.clone().unwrap());
                        SshWizardStep::Shell
                    },
                    ...
                    SudoResult::NoAuthenticatorCodeSupplied => {
                        info!("login sudo skipped (user={})", self.state.email.clone().unwrap());
                        SshWizardStep::Shell
                    }
                    _ => {
            ...
            SshWizardStep::SignUp => {
                ...
                if let Some(response) = response {
                    if response.len() == 2 {
                        ...
                        if password.len() < 4 {
                            self.state.goodbye_message = Some(CConst::SSH_PASSWORD_WEAK.to_string());
                            self.step = SshWizardStep::Terminate;
                            return;
                        }
                ...
                self.step = match user_create(&self.registry, &self.auth, &mut self.state).await {
                    UserCreateResult::Created => {
                        info!("sign up successful (user={})", self.state.email.clone().unwrap());
                        SshWizardStep::Verify
                    }
                    UserCreateResult::Terms => {
                        info!("sign up successful - new terms (user={})", self.state.email.clone().unwrap());
                        SshWizardStep::Terms
                    }
                    UserCreateResult::AlreadyExists => {
                        warn!("sign-up failed: user already exists (user={})", self.state.email.clone().unwrap());
                        self.state.goodbye_message = Some(CConst::SSH_ACCOUNT_EXISTS.to_string());
                        SshWizardStep::Terminate
                    }
                    UserCreateResult::InvalidEmail => {
                        warn!("sign-up failed: username is invalid (user={})", self.state.email.clone().unwrap());
                        self.state.goodbye_message = Some(CConst::SSH_INVALID_USERNAME.to_string());
                        SshWizardStep::Terminate
                    }
            ...
            SshWizardStep::Terms => {
                ...
                self.step = match user_create(&self.registry, &self.auth, &mut self.state).await {
                    UserCreateResult::Created => {
                        info!("terms accepted (user={})", self.state.email.clone().unwrap());
                        SshWizardStep::Verify
                    }
                    UserCreateResult::Terms => {
                        info!("new terms and conditions (user={})", self.state.email.clone().unwrap());
                        SshWizardStep::Terms
                    },
                    _ => {
            ...
            SshWizardStep::Verify => {
                ...
                self.step = match login(&self.registry, &self.auth, &mut self.state).await {
                    ...
                    LoginResult::IncorrectPassword => {
                        ...
                        self.state.goodbye_message = Some(CConst::SSH_WRONG_VERIFY_CODE.to_string());
                        SshWizardStep::Terminate
                    },
                    LoginResult::AccountLocked => {
        ...
        match self.step {
            SshWizardStep::Login => {
                Auth::Partial {
                    ...
                    instructions: Cow::Owned(self.state.parse_message(CConst::SSH_INSTRUCTIONS_PASSWORD)),
            ...
            SshWizardStep::SignUp => {
                Auth::Partial {
                    name: self.get_welcome().into(),
                    instructions: Cow::Owned(self.state.parse_message(CConst::SSH_INSTRUCTIONS_SIGN_UP)),
                    prompts: Cow::Owned(vec![("Password: ".into(), false), ("Password Again: ".into(), false)])
                }
            },
            SshWizardStep::Terms => {
                Auth::Partial {
            ...
            SshWizardStep::Verify => {
                ...
                if self.state.qr_code.is_some() {
                    instructions.push_str(self.state.parse_message(CConst::SSH_INSTRUCTIONS_QR).as_str());
                }
                instructions.push_str(self.state.parse_message(CConst::SSH_INSTRUCTIONS_VERIFY).as_str());
                Auth::Partial {
                    name: self.get_welcome().into(),
...
impl term_api::WizardAbi
for SshWizard
{
    async fn process(&mut self, responses: Vec<String>) -> term_api::WizardAction {
        let responses = if responses.len() > 0 { Some(responses) } else { None };
        match self.next_auth(responses).await {
            ...
            Auth::Partial { name, instructions, prompts } => {
                term_api::WizardAction::Challenge {
                    ...
                    prompts: prompts.iter().map(|(prompt, echo)| {
                        term_api::WizardPrompt {
    ...
    fn token(&self) -> Option<String> {
        self.state.session.clone().map(|a| ate_auth::helper::session_to_b64(a).unwrap())
    }
}

async fn login(registry: &Arc<Registry>, auth: &url::Url, state: &mut SshWizardState) -> LoginResult {
    ...
    let username = match state.email.clone() {
        Some(a) => a,
        None => { return LoginResult::InvalidEmail;  }
    };
    let password = match state.password.clone() {
        Some(a) => a,
        None => { return LoginResult::NoPasswordSupplied;  }
    };
...
async fn user_exists(registry: &Arc<Registry>, auth: &url::Url, state: &mut SshWizardState) -> UserExistsResult {
    // Open a command chain
    let chain = match registry.open_cmd(&auth).await {
        Ok(a) => a,
        Err(err) => {
    ...
    let username = match state.email.clone() {
        Some(a) => a,
        None => { return UserExistsResult::InvalidEmail;  }
    };
...
async fn user_create(registry: &Arc<Registry>, auth: &url::Url, state: &mut SshWizardState) -> UserCreateResult
{
    // Grab the two main parts
    let username = match state.email.clone() {
        Some(a) => a,
        None => { return UserCreateResult::NoUsernameSupplied; }
    };
    let password = match state.password.clone() {
        Some(a) => a,
        None => { return UserCreateResult::NoPasswordSupplied; }
    };

    // Create a user using the authentication server which will give us a session with all the tokens
    let result = match ate_auth::cmd::create_user_command(
        registry,
        username,
        password,
        auth.clone(),
