...
pub fn test_compact_state_machine() -> Result<(), AteError> {
    ...
    rt.block_on(async
    {
        ...
        let wait = rx.wait_for_compact();
        let _tx = TaskEngine::spawn(async move { tokio::time::sleep(Duration::from_millis(10)).await; let _ = tx.log_size.send(100u64); tx });
        ...
        let _tx = TaskEngine::spawn(async move { tokio::time::sleep(Duration::from_millis(10)).await; let _ = tx.log_size.send(1100u64); tx });
        timeout(Duration::from_millis(50), wait).await
            .expect_err("The growth size event should not be triggered");
        ...
        let (tx, mut rx) = CompactState::new(CompactMode::GrowthSize(500), 1000);
        let wait = rx.wait_for_compact();
        let _tx = TaskEngine::spawn(async move { tokio::time::sleep(Duration::from_millis(10)).await; let _ = tx.log_size.send(1400u64); let _ = tx.log_size.send(1600u64); tx });
        timeout(Duration::from_millis(100), wait).await
            .expect("This should not timeout")?;
        ...
        let wait = rx.wait_for_compact();
        let start = Instant::now();
        let _tx = TaskEngine::spawn(async move { tokio::time::sleep(Duration::from_millis(10)).await; let _ = tx.log_size.send(2000u64); tx });
        timeout(Duration::from_millis(50), wait).await
