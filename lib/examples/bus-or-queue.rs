...
async fn main() -> Result<(), AteError>
{
    ...
    let _root = create_mesh(&cfg_ate,&cfg_mesh).await;
    ...
    let chain_a = client_a.persistent(ChainKey::from("ping-pong-table")).await.unwrap();
    let (mut bus, key) =
    {
...

    {
        ...
        let chain_b = client_b.persistent(ChainKey::from("ping-pong-table")).await.unwrap();
        chain_b.sync().await?;
