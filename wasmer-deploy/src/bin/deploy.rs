...
#[allow(dead_code)]
#[derive(Parser)]
#[clap(version = "1.2", author = "Wasmer Inc <info@wasmer.io>")]
struct Opts {
    ...
    pub token_path: String,
    /// URL that this command will send all its authentication requests to (e.g. wss://wasmer.sh/auth)
    #[clap(long)]
...
enum SubCommand {
    ...
    /// anywhere via API calls and/or the wasmer-bus.
    #[clap()]
...
async fn init_wasi_ws() {
    // Add the main Wasmer certificate and connect via a wasmer_bus web socket
    ...
    set_comm_factory(move |addr| {
        ...
        Box::pin(async move {
            tracing::trace!("opening wasmer_bus::web_socket");
            let ws = wasmer_bus_ws::prelude::SocketBuilder::new(addr)
                .open()
                .await
                .unwrap();
            let (tx, rx) = ws.split();
...
async fn main_async() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut opts = {
        let cmd = match binary_as_util {
            ...
            "net" => Some(SubCommand::Network(OptsNetwork::parse())),
            "network" => Some(SubCommand::Network(OptsNetwork::parse())),
    ...
    let auth = wasmer_auth::prelude::origin_url(&opts.auth_url, "auth");
    ...
    match opts.subcmd {
        ...
        SubCommand::Instance(opts_instance) => {
            let db_url = wasmer_auth::prelude::origin_url(&opts_instance.db_url, "db");
            let inst_url = wasmer_auth::prelude::origin_url(&opts_instance.inst_url, "inst");
