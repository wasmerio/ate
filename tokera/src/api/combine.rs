...
impl TokApi
{
    pub(super) async fn __combine_coins(&mut self) -> Result<(), CoinError>
    {
        ...
        for (_, mut bag) in bags
        {
            ...
            let coins_per_zipped_bag_plus_plus = ((COINS_PER_STACK_TO_BE_COMBINED * 12usize) / 10usize) + 1usize;
            ...
            trace!("built a bag to combine with {} coins", bag_to_combine.coins.len());
            ...
            let temp_ownership = {
                ...
                trace!("adding ({}) ownership(s) as fallback coins", ownership.len());
                for ownership in ownership {
                    to.push(
                        wallet_mut.inbox.push_with_dio(&self.dio, ownership)?
                    );
                }
                to.push(
                    wallet_mut.inbox.push_with_dio(&self.dio, new_ownership.clone())?
                );
                to
            };
            wallet_mut.commit()?;
            ...
            let res = coin_combine_command(&self.registry, bag_to_combine.coins, new_ownership, self.auth.clone()).await?;
            ...
            wallet_mut.bags.get_or_default(denomination).await?.as_mut().coins.push(res.super_coin);
            wallet_mut.commit()?;
