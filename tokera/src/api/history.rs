...
impl TokApi {
    ...
    pub async fn get_or_create_today(&mut self) -> Result<DaoMut<HistoricDay>, WalletError> {
        ...
        let ret = this_month.days.push_with_dio(
            &self.dio,
            HistoricDay {
                day: now.day(),
                activities: Vec::new(),
    ...
    ) -> Result<DaoMut<HistoricDay>, WalletError> {
    ...
    pub async fn read_activity(
        ...
        filter_month: Option<u32>,
        filter_day: Option<u32>,
    ) -> Result<Vec<HistoricActivity>, WalletError> {
        let mut ret = Vec::new();
