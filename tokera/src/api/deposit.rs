...
impl TokApi
{
    pub async fn deposit_create(&mut self, currency: NationalCurrency, amount: Decimal) -> Result<DepositResponse, WalletError>
    {
        // Invoke the command
        let session = self.dio.session().clone_session();
        let ret = deposit_command(&self.registry, amount, currency, session.deref(), self.auth.clone()).await?;

        // Load the new set of coins into the wallet
        self.wallet.inbox.push_with_dio(&self.dio, ret.coin_ancestor.clone())?;

        // Now add the history
        if let Err(err) = self.record_activity(HistoricActivity::DepositCreated(activities::DepositCreated {
    ...
    {
        let things = self.wallet.inbox.iter().await?
            .filter(|a| {
    ...
    pub(super) async fn __deposit_cancel(&mut self, invoice_number: &str) -> Result<(), WalletError>
    {
        let coins = self.wallet.inbox.iter().await?
            .filter(|a| {
                if let CommodityCategory::NationalCurrency(_) = a.kind.category() {
                    true
                } else {
        ...
        let ancestor = query.pending_deposits.iter()
        ...
        let ancestor = match ancestor {
            Some(a) => a,
            None => {
                bail!(WalletErrorKind::CoinError(CoinErrorKind::InvalidReference(invoice_number.to_string())));
            }
        };
        let to_delete = self.wallet.as_mut().inbox.iter_mut().await?
            .filter(|a| ancestor.key.eq(&a.what))
            .next()
            .unwrap();

        let _ = cancel_deposit_command(&self.registry, to_delete.clone().take(), self.auth.clone()).await?;
