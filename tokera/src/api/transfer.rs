...
impl TokApi
{
    pub async fn deposit_coins(&mut self, coins: BagOfCoins, notify: Option<CoinRotateNotification>)
    -> std::result::Result<CoinRotateResponse, WalletError>
    {
        ...
        let ret = coin_rotate_command(&self.registry, coins.coins, new_token, session.deref(), self.auth.clone(), notify).await?;
    ...
    pub async fn transfer<A, B>(&mut self, amount: Decimal, currency: NationalCurrency, destination: &dyn OptsPurpose<A>, source: &dyn OptsPurpose<B>, should_notify: bool)
    -> std::result::Result<CoinRotateResponse, WalletError>
    where A: Clone,
          B: Clone
    {
        ...
        let ret = self.__transfer::<A, B>(amount, currency, destination, source, should_notify).await;
    ...
    pub(super) async fn __transfer<A, B>(&mut self, amount: Decimal, currency: NationalCurrency, destination: &dyn OptsPurpose<A>, source: &dyn OptsPurpose<B>, should_notify: bool)
    -> std::result::Result<CoinRotateResponse, WalletError>
    where A: Clone,
          B: Clone
    {
        ...
        let mut api_destination = {
            ...
            let session: AteSessionType = if let Purpose::<A>::Domain { domain_name: group_name, wallet_name: _, action: _} = destination.purpose() {
                gather_command(&self.registry, group_name.clone(), session, self.auth.clone()).await?.into()
            } else {
                session.into()
            };
        ...
        // Now add the history into both wallets
        {
            ...
            if let Err(err) = self.record_activity(HistoricActivity::TransferOut(activity.clone())).await {
                error!("Error writing activity: {}", err);
            }

            match api_destination.record_activity(HistoricActivity::TransferIn(activity.clone())).await {
                Ok(_) => {
