            ...
            async move {
                ...
                // Add all the operations
                {
                    ...
                    respond_to(
                        handle,
                        SerializationFormat::Json,
                        move |_handle, meta: api::FileSystemReadMetadataRequest| {
                            debug!("bus::read-metadata(path={})", meta.path);
                            let accessor = accessor.clone();
                            async move {
                                if let Ok(Some(file)) =
                                    accessor.search(&context, meta.path.as_str()).await
                                {
                ...
                }
                {
                    ...
                    respond_to(
                        handle,
                        SerializationFormat::Json,
                        move |_handle, meta: api::FileSystemReadSymlinkMetadataRequest| {
                ...
                }
                {
                    ...
                    respond_to(
                        handle,
                        SerializationFormat::Json,
                        move |_handle, read_dir: api::FileSystemReadDirRequest| {
                            ...
                            async move {
                                    ...
                                    accessor.search(&context, read_dir.path.as_str()).await
                                {
                                        ...
                                        accessor.opendir(&context, file.ino, O_RDONLY as u32).await
                                    {
                                        ...
                                        let _ =
                                            accessor.releasedir(&context, file.ino, fh.fh, 0).await;
                                        FsResult::Ok(ret)
                                    } else {
                ...
                }
                {
                    ...
                    respond_to(
                        handle,
                        SerializationFormat::Json,
                        move |_handle, create_dir: api::FileSystemCreateDirRequest| {
                ...
                }
                {
                    ...
                    respond_to(
                        handle,
                        SerializationFormat::Json,
                        move |_handle, remove_dir: api::FileSystemRemoveDirRequest| {
                ...
                }
                {
                    ...
                    respond_to(
                        handle,
                        SerializationFormat::Json,
                        move |_handle, rename: api::FileSystemRenameRequest| {
                            ...
                            async move {
                                ...
                                let name = path.file_name().ok_or_else(|| FsError::InvalidInput)?;
                                let new_name =
                                    new_path.file_name().ok_or_else(|| FsError::InvalidInput)?;
                                let parent = path.parent().ok_or_else(|| FsError::InvalidInput)?;
                                let new_parent =
                                    new_path.parent().ok_or_else(|| FsError::InvalidInput)?;
                                if let Ok(Some(parent)) = accessor
                                    .search(&context, parent.to_string_lossy().as_ref())
                                    .await
                                {
                ...
                }
                {
                    ...
                    respond_to(
                        handle,
                        SerializationFormat::Json,
                        move |_handle, remove_file: api::FileSystemRemoveFileRequest| {
                ...
                }
                {
                    ...
                    respond_to(
                        handle,
                        SerializationFormat::Json,
                        move |handle, new_open: api::FileSystemOpenRequest| {
                            let accessor = accessor.clone();
                            async move {
                                // Determine all the flags
                                let mut flags = 0i32;
                                if new_open.options.append {
                                    flags |= O_APPEND;
                                }
                                if new_open.options.create {
                                    flags |= O_CREAT;
                                }
                                if new_open.options.create_new {
                                    flags |= O_CREAT | O_TRUNC;
                                }
                                if new_open.options.read && new_open.options.write {
                                    flags |= O_RDWR;
                                } else if new_open.options.read {
                                    flags |= O_RDONLY;
                                } else if new_open.options.write {
                                    flags |= O_WRONLY;
                                }
                                if new_open.options.truncate {
                                    flags |= O_TRUNC;
                                }
                                let append = new_open.options.append;
                                let create = new_open.options.create | new_open.options.create_new;

                                // Open the file!
                                let file = if let Ok(Some(file)) =
                                    accessor.search(&context, new_open.path.as_str()).await
                                {
                                    ...
                                    let path = std::path::Path::new(&new_open.path);
                                    let name =
                                        path.file_name().ok_or_else(|| FsError::InvalidInput);
                                    match name {
                                        Ok(name) => {
                                            let parent =
                                ...
                                let offset = if append {
                                    if let Ok(file) = file.as_ref() {
                                        Arc::new(Mutex::new(file.spec.size() as u64))
                                    } else {
                                ...
                                let path = new_open.path.clone();
                                let ios = Arc::new(Mutex::new(Vec::new()));
                                let (tx_close, mut rx_close) = mpsc::channel::<()>(1);
                                {
                                    ...
                                    respond_to(
                                        handle,
                                        SerializationFormat::Json,
                                        move |_handle, _: api::OpenedFileIoRequest| {
                                            debug!("bus::io");

                                            let (tx_io_close, mut rx_io_close) =
                                                mpsc::channel::<()>(1);
                                            ios.lock().unwrap().push(tx_io_close);
                                            ...
                                            async move {
                                                {
                                                    let offset = offset.clone();
                                                    let file = file.clone();
                                                    respond_to(handle, SerializationFormat::Bincode, move |_handle, seek: api::FileIoSeekRequest| {
                                                debug!("bus::seek({:?})", seek);
                                                let offset = offset.clone();
                                                let file = file.clone();
                                                async move {
...

                                                {
                                                    let offset = offset.clone();
                                                    let file = file.clone();
                                                    respond_to(handle, SerializationFormat::Bincode, move |_handle, write: api::FileIoWriteRequest| {
...

                                                {
                                                    let offset = offset.clone();
                                                    let file = file.clone();
                                                    respond_to(handle, SerializationFormat::Bincode, move |_handle, read: api::FileIoReadRequest| {
                                                ...
                                                {
                                                    let file = file.clone();
                                                    respond_to(handle, SerializationFormat::Bincode, move |_handle, _flush: api::FileIoFlushRequest| {
                                                debug!("bus::flush");
                                                let file = file.clone();
                                                async move {
                                                    let file = file?;
...

                                {
                                    ...
                                    respond_to(
                                        handle,
                                        SerializationFormat::Json,
                                        move |_handle, _unlink: api::OpenedFileUnlinkRequest| {
...

                                {
                                    ...
                                    respond_to(
                                        handle,
                                        SerializationFormat::Json,
                                        move |_handle, set_length: api::OpenedFileSetLenRequest| {
                                ...
                                // The file will shutdown when an close command is received
                                {
                                    ...
                                    respond_to(
                                        handle,
                                        SerializationFormat::Json,
                                        move |_handle, _close: api::OpenedFileCloseRequest| {
                                            debug!("bus::close");
                                            let file = file.clone();
                                ...
                                let ios = ios.lock().unwrap().drain(..).collect::<Vec<_>>();
                                for io in ios {
                                    let _ = io.send(()).await;
                                }
                ...
                respond_to(
                    handle,
                    SerializationFormat::Json,
                    move |_handle, _meta: api::FileSystemUnmountRequest| {
