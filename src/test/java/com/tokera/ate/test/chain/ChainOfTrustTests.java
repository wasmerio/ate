...
public class ChainOfTrustTests
{
    ...
    public void addMany() throws IOException, InvalidCipherTextException
    {
        ...
        boolean accepted = chain.rcv(data.createBaseFlatBuffer(), new MessageMetaDto(0, index++), true, LOG);
        ...
        for (int n = 0; n < 200; n++)
        {
            ...
            for (int x = 0; x < 100; x++) {
                ...
                accepted = chain.rcv(data.createBaseFlatBuffer(), new MessageMetaDto(0, index++), true, LOG);
                Assertions.assertTrue(accepted);
            }
        }
        ...
        data = new MessageDataDto(header, digest, bytes1);
        chain.rcv(data.createBaseFlatBuffer(), new MessageMetaDto(0, index++), true, LOG);
