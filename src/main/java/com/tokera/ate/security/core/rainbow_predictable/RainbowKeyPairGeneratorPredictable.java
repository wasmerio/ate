 ...
 */
public class RainbowKeyPairGeneratorPredictable
        implements AsymmetricCipherKeyPairGenerator
{
    ...
    public AsymmetricCipherKeyPair genKeyPair()
    {
        ...
        if (!initialized)
        {
            throw new RuntimeException("You must initialize the system first.");
        }
    ...
    private void generateL1()
    {
        ...
        while (A1inv == null)
        {
            for (int i = 0; i < dim; i++)
            {
                for (int j = 0; j < dim; j++)
                {
                    A1[i][j] = (short)(sr.getRandom().nextInt() & GF2Field.MASK);
                }
            }
            A1inv = c.inverse(A1);
        }
        ...
        for (int i = 0; i < dim; i++)
        {
            b1[i] = (short)(sr.getRandom().nextInt() & GF2Field.MASK);
        }
    }
    ...
    private void generateL2()
    {
        ...
        while (this.A2inv == null)
        {
            for (int i = 0; i < dim; i++)
            {
                for (int j = 0; j < dim; j++)
                { // one col extra for b
                    A2[i][j] = (short)(sr.getRandom().nextInt() & GF2Field.MASK);
                }
            }
        ...
        for (int i = 0; i < dim; i++)
        {
            b2[i] = (short)(sr.getRandom().nextInt() & GF2Field.MASK);
        }
    ...
    private void generateF()
    {
        ...
        {
            layers[i] = generateLayer(this.vi[i], this.vi[i + 1]);
        }
    }

    private Layer generateLayer(int vi, int viNext) {
        int oi = viNext - vi;

        // the coefficients of all polynomials in this layer
        short [][][] coeff_alpha = new short[oi][oi][vi];
        short [][][] coeff_beta = new short[oi][vi][vi];
        short [][] coeff_gamma = new short[oi][viNext];
        short [] coeff_eta = new short[oi];

        int numOfPoly = oi; // number of polynomials per layer
        ...
        for (int k = 0; k < numOfPoly; k++)
        {
            for (int i = 0; i < oi; i++)
            {
                for (int j = 0; j < vi; j++)
                {
                    coeff_alpha[k][i][j] = (short)(sr.getRandom().nextInt() & GF2Field.MASK);
                }
            }
        }
        // Beta coeffs
        for (int k = 0; k < numOfPoly; k++)
        {
            for (int i = 0; i < vi; i++)
            {
                for (int j = 0; j < vi; j++)
                {
                    coeff_beta[k][i][j] = (short)(sr.getRandom().nextInt() & GF2Field.MASK);
                }
            }
        }
        // Gamma coeffs
        for (int k = 0; k < numOfPoly; k++)
        {
            for (int i = 0; i < viNext; i++)
            {
                coeff_gamma[k][i] = (short)(sr.getRandom().nextInt() & GF2Field.MASK);
            }
        }
        // Eta
        for (int k = 0; k < numOfPoly; k++)
        {
            coeff_eta[k] = (short)(sr.getRandom().nextInt() & GF2Field.MASK);
        }
