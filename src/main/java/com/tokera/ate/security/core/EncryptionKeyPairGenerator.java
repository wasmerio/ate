...
public class EncryptionKeyPairGenerator {
    ...
    public @NonNull AsymmetricCipherKeyPair generateKeyPair(IRandom random) {
        ...
        while (true) {
            ...
            if (fastFp) {
                t = params.polyType == NTRUParameters.TERNARY_POLYNOMIAL_TYPE_SIMPLE ? PredictableSupportUtil.generateRandomTernary(N, df, df, sparse, random.getRandom()) : PredictableSupportUtil.generateRandomProduct(N, df1, df2, df3, df3, random.getRandom());
                f = t.toIntegerPolynomial();
                f.mult(3);
                f.coeffs[0] += 1;
            } else {
                t = params.polyType == NTRUParameters.TERNARY_POLYNOMIAL_TYPE_SIMPLE ? PredictableSupportUtil.generateRandomTernary(N, df, df - 1, sparse, random.getRandom()) : PredictableSupportUtil.generateRandomProduct(N, df1, df2, df3, df3 - 1, random.getRandom());
                f = t.toIntegerPolynomial();
                fp = f.invertF3();
                if (fp == null) {
                    continue;
        ...
        while (true) {
            g = PredictableSupportUtil.generateRandomDense(N, dg, dg - 1, random.getRandom());
