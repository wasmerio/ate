...
impl Reactor
{
    ...
    pub fn generate_fd_with_id(&mut self, fd: RawFd) -> Result<RawFd, i32> {
        if self.fd_pipe_rx.contains_key(&fd) == false &&
            self.fd_pipe_tx.contains_key(&fd) == false
        {
            return Ok(fd);
        }
        Err(ERR_EMFILE)
    ...
    pub fn bidirectional(&mut self, buffer_size_tx: usize, buffer_size_rx: usize, mode: ReceiverMode) -> Result<(RawFd, mpsc::Sender<Vec<u8>>, mpsc::Receiver<Vec<u8>>), i32> {
        let fd = self.generate_fd()?;
        let (tx_send, rx_send) = mpsc::channel(buffer_size_tx);
        let (tx_recv, rx_recv) = mpsc::channel(buffer_size_rx);
        self.fd_pipe_rx.insert(fd, Arc::new(Mutex::new(ReactorPipeReceiver {
            rx: rx_send,
            buffer: BytesMut::new(),
            mode,
        })));
    ...
    pub fn pipe_in_with_fd(&mut self, fd: RawFd, mode: ReceiverMode) -> Result<(RawFd, mpsc::Sender<Vec<u8>>),i32> {
        let (tx, rx) = mpsc::channel(MAX_MPSC);
    ...
    pub fn generate_job(&mut self, stdio: Stdio, stdin_tx: mpsc::Sender<Vec<u8>>) -> Result<(u32, Job), i32> {
        let mut job_seed = 1;
        for _ in 0..10000 {
            let id = job_seed;
            job_seed += 1;
