...
impl FileSystem
{
    pub fn mount(wapm: &str, name: &str) -> FileSystem {
        ...
        let task = call(wapm.to_string().into(), mount)
            .invoke();
        FileSystem {
            task
        }
    ...
    async fn read_dir(&self, path: &Path) -> FsResult<backend::Dir> {
        ...
        self.task.call(backend::ReadDir {
    ...
    async fn create_dir(&self, path: &Path) -> FsResult<()> {
        ...
        self.task.call(backend::CreateDir {
            path: path.to_string_lossy().to_string(),
        })
        .invoke().join().await
        .map_err(|_| backend::FsError::IOError)?
    ...
    async fn remove_dir(&self, path: &Path) -> FsResult<()> {
        ...
        self.task.call(backend::RemoveDir {
            path: path.to_string_lossy().to_string(),
        })
        .invoke().join().await
        .map_err(|_| backend::FsError::IOError)?
    ...
    async fn metadata(&self, path: &Path) -> FsResult<backend::Metadata> {
        ...
        self.task.call(backend::ReadMetadata {
            path: path.to_string_lossy().to_string(),
        })
        .invoke().join().await
        .map_err(|_| backend::FsError::IOError)?
    ...
    async fn symlink_metadata(&self, path: &Path) -> FsResult<backend::Metadata> {
        ...
        self.task.call(backend::ReadSymlinkMetadata {
            path: path.to_string_lossy().to_string(),
        })
        .invoke().join().await
        .map_err(|_| backend::FsError::IOError)?
    ...
    async fn remove_file(&self, path: &Path) -> FsResult<()> {
        ...
        self.task.call(backend::RemoveFile {
            path: path.to_string_lossy().to_string(),
        })
        .invoke().join().await
        .map_err(|_| backend::FsError::IOError)?
...
impl OpenOptions {
    ...
    pub async fn open(&mut self, path: &Path) -> FsResult<VirtualFile> {
        let task = self.fs.task.call(backend::NewOpen {
            read: self.conf.read,
        ...
        let meta = task.call(backend::Open {
            path: path.to_string_lossy().to_string(),
        })
        .invoke().join::<FsResult<backend::Metadata>>()
        .await
        .map_err(|_| backend::FsError::IOError)??;
...
impl Read for VirtualFile {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        let data: Result<Vec<u8>, io::Error> = self.task.call(backend::Flush { })
            .invoke()
...
impl VirtualFile {
    ...
    async fn set_len(&mut self, new_size: u64) -> FsResult<()> {
        let result: FsResult<()> = self.task.call(backend::SetLength {
            len: new_size
        })
        .invoke().join().await
        .map_err(|_| backend::FsError::IOError)?;
